...
const sumTodayWaterIntake = (waterLog = []) => {
  const today = getTodayDateString();
  return waterLog.reduce((sum, entry) => {
...
const getExerciseTotals = (user) => {
  ...
  return {
    ...
    cardio: todaysExercises.filter((exercise) => exercise.exerciseType === "Cardio").length,
    other: todaysExercises.filter((exercise) => exercise.exerciseType === "Other").length
  };
};

const getCaloriesFromFoods = (foods = []) => {
  return foods.reduce((sum, food) => {
    const calories = Number(food.calories);
    const servings = Number(food.servings);
    const safeCalories = Number.isFinite(calories) ? calories : 0;
    const safeServings = Number.isFinite(servings) ? servings : 0;
    return sum + (safeCalories * safeServings);
  }, 0);
};
...
const getDefaultJarvisReply = (prompt, goals, exercises, calories, water) => {
  if (prompt.includes("water") || prompt.includes("hydrate")) {
    ...
    return remaining > 0
      ? `Hydration protocol active. You need ${remaining} ml more water to hit your daily target of ${goals.dailyWaterTarget} ml.`
      : "Hydration target achieved. Excellent discipline, Avenger.";
  }
  ...
  if (prompt.includes("nutrition") || prompt.includes("calorie") || prompt.includes("food")) {
    return calories === 0
      ? "No nutrition entries logged today. Add your next meal to keep your mission analytics accurate."
...
const getGroqJarvisReply = async ({ prompt, goals, exercises, calories, water }) => {
  ...
  const userPrompt = [
    ...
    `Weekly workout target: ${goals.weeklyWorkoutTarget}`
  ].join("\n");
...
/* PUT - save avenger training goals */
router.put("/fitnessGoals/:userId", verify, async (req, res) => {
  try {
    const userId = req.params.userId;
    const user = await User.findById(userId);
    ...
    const parsedWorkoutTarget = Number(weeklyWorkoutTarget);
    const parsedWaterTarget = Number(dailyWaterTarget);

    if (
      !missionName ||
      !Number.isFinite(parsedWorkoutTarget) ||
      !Number.isFinite(parsedWaterTarget) ||
      parsedWorkoutTarget <= 0 ||
      parsedWaterTarget <= 0
    ) {
      return res.status(400).json({ error: "Invalid goals provided" });
    }

    user.lowLevelFitnessGoals = [{
      missionName,
      weeklyWorkoutTarget: parsedWorkoutTarget,
      dailyWaterTarget: parsedWaterTarget
    }];

    await user.save();
    return res.status(200).json(user.lowLevelFitnessGoals[0]);
  } catch (error) {
...
/* POST - JARVIS AI mission assistant */
router.post("/jarvis/:userId", verify, async (req, res) => {
  try {
    const userId = req.params.userId;
    const user = await User.findById(userId);
    ...
    const prompt = (req.body.message || "").toLowerCase().trim();
    const goals = Array.isArray(user.lowLevelFitnessGoals) && user.lowLevelFitnessGoals.length > 0
      ? user.lowLevelFitnessGoals[0]
    ...
    try {
      ...
      console.log("GROQ JARVIS fallback:", error.message);
    }

    return res.status(200).json({
      reply: response,
      stats: {
        exercisesToday: exercises.total,
...
/* GET - get avenger dashboard stats */
router.get("/dashboard/:userId", verify, async (req, res) => {
  try {
    ...
    const exercises = getExerciseTotals(user);
    const caloriesToday = getCaloriesFromFoods(user.foods);
    const waterToday = sumTodayWaterIntake(user.waterIntakeLog);

    return res.status(200).json({
      missionName: goals.missionName,
      weeklyWorkoutTarget: Number(goals.weeklyWorkoutTarget) || 5,
      dailyWaterTarget: Number(goals.dailyWaterTarget) || 3000,
      workoutsToday: exercises.total,
      caloriesToday,
      waterToday
    });
  } catch (error) {
...
/* GET - get fitness goals */
router.get("/fitnessGoals/:userId", verify, async (req, res) => {
  try {
    const userId = req.params.userId;
    const user = await User.findById(userId);
