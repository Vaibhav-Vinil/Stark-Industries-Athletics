...
const generateUniquePhone = async () => {
    ...
    do {
        candidate = `9${Math.floor(100000000 + Math.random() * 900000000)}`;
        existingUser = await User.findOne({ phone: candidate });
    } while (existingUser);
...
router.post("/register", async (req, res) => {
    ...
    const user = await User.findOne({
        $or: [
            ...
            { phone: req.body.phone || "__unused_phone__" },
    ...
    /* Create new user */
    const generatedPhone = req.body.phone || await generateUniquePhone();
