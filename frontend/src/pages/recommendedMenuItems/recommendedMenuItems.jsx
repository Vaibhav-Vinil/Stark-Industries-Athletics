...
const RecommendedMenuItems = () => {
    const FALLBACK_RECOMMENDED_ITEMS = [
        { ID: "rec-1", name: "Turkey Avocado Wrap", avgRating: 4.3 },
        { ID: "rec-2", name: "Mediterranean Chickpea Bowl", avgRating: 4.3 },
        { ID: "rec-3", name: "High-Protein Chili", avgRating: 4.4 }
    ];
    ...
    async function getRecommendationsBasedOnSavedItems(mealType) {
        try {
            ...
            if (response.data.length === 0) {
                ...
                setCourtsMenu(FALLBACK_RECOMMENDED_ITEMS);
                setRecsSaved(FALLBACK_RECOMMENDED_ITEMS);
                return;
            }
    ...
    async function getRecommendationsBasedOnPrefsRests(mealType) {
        try {
            ...
            if (prefs.length === 0 && rests.length === 0) {
                loading.current = false // not loading
                setCourtsMenu(FALLBACK_RECOMMENDED_ITEMS);
                setRecsPrefsRests(FALLBACK_RECOMMENDED_ITEMS);
                return;
            }
