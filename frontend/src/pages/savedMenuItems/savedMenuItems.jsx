...
const SavedMenuItems = () => {
    const FALLBACK_SAVED_ITEMS = [
        { ID: "saved-1", name: "Grilled Chicken Power Bowl", avgRating: 4.6 },
        { ID: "saved-2", name: "Salmon and Quinoa Plate", avgRating: 4.7 },
        { ID: "saved-3", name: "Greek Yogurt Fruit Cup", avgRating: 4.5 }
    ];
    ...
    useEffect(() => {
        const getSavedItems = async () => {
            try {
                ...
                const courtsItems = response.data.length ? response.data : FALLBACK_SAVED_ITEMS;
                loading.current = false /* not loading anymore after items are loaded */
                setCourtsMenu(courtsItems);
            } catch (error) { 
                loading.current = false /* not loading anymore after items are loaded */
                setCourtsMenu(FALLBACK_SAVED_ITEMS);
                console.log(error);
            };
        };
