...
const Menu = () => {
    ...
    useEffect(() => {
        ...
        if (shouldSort) {
            ...
            const beforeSort = [...courtsMenu];
            setMenuBeforeSort(beforeSort); // unsorted items now stored in menuBeforeSort
            const sorted = [...courtsMenu].sort((a, b) =>
                a.name > b.name ? 1 : b.name > a.name ? -1 : 0
            );
            setCourtsMenu(sorted);
        } else if (!shouldSort) {
    ...
    useEffect(() => {
        ...
        if (shouldSortPop) {
            ...
            setDisableSortFat(true);
            setDisableSortCalories(true);
            setMenuBeforeSortPop([...courtsMenu]);
            const sorted = [...courtsMenu].sort((a, b) =>
    ...
    useEffect(() => {
        if (shouldSortProtein) {
            ...
            const sorted = [...courtsMenu].sort((a, b) => Number(b?.nutritionFacts?.[11]?.Value || 0) - Number(a?.nutritionFacts?.[11]?.Value || 0));
            setCourtsMenu(sorted);
    ...
    useEffect(() => {
        if (shouldSortFat) {
            ...
            const sorted = [...courtsMenu].sort((a, b) => Number(a?.nutritionFacts?.[3]?.Value || 0) - Number(b?.nutritionFacts?.[3]?.Value || 0));
            setCourtsMenu(sorted);
      ...
      useEffect(() => {
        if (shouldSortCalories) {
            ...
            const sorted = [...courtsMenu].sort((a, b) => Number(a?.nutritionFacts?.[1]?.Value || 0) - Number(b?.nutritionFacts?.[1]?.Value || 0));
            setCourtsMenu(sorted);
    ...
    const FALLBACK_MENU_BY_COURT = {
        Windsor: [
            { ID: "windsor-1", name: "Grilled Chicken Power Bowl", avgRating: 4.6 },
            { ID: "windsor-2", name: "Turkey Avocado Wrap", avgRating: 4.3 },
            { ID: "windsor-3", name: "Veggie Omelet Plate", avgRating: 4.2 }
        ],
        Wiley: [
            { ID: "wiley-1", name: "Lean Beef Burrito Bowl", avgRating: 4.4 },
            { ID: "wiley-2", name: "Tofu Stir Fry", avgRating: 4.1 },
            { ID: "wiley-3", name: "Greek Yogurt Fruit Cup", avgRating: 4.5 }
        ],
        Ford: [
            { ID: "ford-1", name: "Salmon and Quinoa Plate", avgRating: 4.7 },
            { ID: "ford-2", name: "Protein Pasta Primavera", avgRating: 4.2 },
            { ID: "ford-3", name: "Egg White Breakfast Wrap", avgRating: 4.0 }
        ],
        Earhart: [
            { ID: "earhart-1", name: "Chicken Caesar Salad", avgRating: 4.1 },
            { ID: "earhart-2", name: "High-Protein Chili", avgRating: 4.4 },
            { ID: "earhart-3", name: "Brown Rice Veg Bowl", avgRating: 4.0 }
        ],
        Hillenbrand: [
            { ID: "hill-1", name: "Steak and Sweet Potato", avgRating: 4.8 },
            { ID: "hill-2", name: "Mediterranean Chickpea Bowl", avgRating: 4.3 },
            { ID: "hill-3", name: "Protein Pancake Stack", avgRating: 4.2 }
        ]
    };
    ...
    const getCourtsItems = async () => {
        try {
            ...
            let courtsItems = response.data;
            if (!courtsItems || courtsItems.length === 0) {
                courtsItems = FALLBACK_MENU_BY_COURT[location] || [];
            }
