...
const PopularMenuItems = () => {
    const FALLBACK_POPULAR_ITEMS = [
        { ID: "popular-1", name: "Steak and Sweet Potato", avgRating: 4.8 },
        { ID: "popular-2", name: "Grilled Chicken Power Bowl", avgRating: 4.6 },
        { ID: "popular-3", name: "Salmon and Quinoa Plate", avgRating: 4.7 }
    ];
        ...
        const getCourtsItems = async () => {
            try {
                ...
                if (response.data.length === 0) {
                    setExistsPopularItems(true);
                }
                const courtsItems = response.data.length ? response.data : FALLBACK_POPULAR_ITEMS;
                loading.current = false; /* done loading */
                setpopularItems(courtsItems);
            } catch (error) { 
                loading.current = false; /* done loading */
                setpopularItems(FALLBACK_POPULAR_ITEMS);
                setExistsPopularItems(true);
                console.log(error) 
            };
        };
